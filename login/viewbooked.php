...
tr:nth-child(even) {background-color: #f2f2f2}
  ...
  <body>
    ...
    <a class="navbar-brand bg-gradient-success"  href="#" id="demo" onmouseover="mouseOver()" onmouseout="mouseOut()"> Event Planner </a>
...
$sql = "SELECT `bid`, `eventtype`, `eventplace`, `no_guests`, `date`, `equip`, `food`, `light`, `flowers`, `chair` FROM `bookevent`";
$result = $conn->query($sql);
if ($result->num_rows() > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["bid"]. "</td><td>" . $row["eventtype"] . "</td><td>"
. $row["eventplace"]. "</td><td>" . $row["no_guests"]. "</td><td>" . $row["date"]. "</td><td>" . $row["equip"]. "</td><td>" . $row["food"]. "</td><td>" . $row["light"]. "</td><td>" . $row["flowers"]. "</td><td>" . $row["chair"]. "</td></tr>";
}} else { echo "0 results"; }
echo "</table>";
